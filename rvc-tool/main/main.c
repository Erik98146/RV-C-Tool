...
#define WIFI_RECONNECT_DELAY_MS 15000
#define WIFI_SCAN_RECONNECT_PAUSE_MS 60000
...
static volatile bool wifi_sta_connecting = false;
...
static volatile bool wifi_ignore_disconnect_event = false;
static volatile uint32_t wifi_connect_generation = 0;
static volatile uint32_t wifi_scan_generation = 0;
...
static bool wifi_timer_stop_if_active(esp_timer_handle_t timer);
static esp_err_t wifi_scan_with_sta_paused(httpd_req_t *req);
...
static bool wifi_timer_stop_if_active(esp_timer_handle_t timer)
{
    ...
    if (!esp_timer_is_active(timer)) return false;

    esp_err_t ret = esp_timer_stop(timer);
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
        ESP_LOGW(TAG, "Wi-Fi timer stop failed: %s", esp_err_to_name(ret));
    }
    return ret == ESP_OK;
}
...
static void wifi_cancel_reconnect(void)
{
    (void)wifi_timer_stop_if_active(wifi_reconnect_timer);
}

static void wifi_schedule_reconnect(uint32_t delay_ms)
{
    if (!wifi_reconnect_enabled) return;
    if (wifi_scan_in_progress) return;
    if (saved_wifi_ssid[0] == '\0') return;
    if (wifi_sta_got_ip || wifi_sta_connecting) return;
    if (wifi_reconnect_timer == NULL) return;

    wifi_timer_stop_if_active(wifi_reconnect_timer);
    esp_err_t ret = esp_timer_start_once(wifi_reconnect_timer, (uint64_t)delay_ms * 1000ULL);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Wi-Fi saved STA reconnect scheduled in %u ms", (unsigned)delay_ms);
    } else if (ret != ESP_ERR_INVALID_STATE) {
        ESP_LOGW(TAG, "Wi-Fi reconnect timer start failed: %s", esp_err_to_name(ret));
    }
}
...
static esp_err_t wifi_sta_apply_and_connect(const char *ssid, const char *password)
{
    ...
    wifi_connect_generation++;
    wifi_cancel_reconnect();

    /* If we are already connected or mid-connect, intentionally disconnect first.
       The disconnect event from this call must not schedule another reconnect. */
    if (wifi_sta_connected || wifi_sta_connecting || wifi_sta_got_ip) {
        wifi_ignore_disconnect_event = true;
...
static void wifi_reconnect_timer_cb(void *arg)
{
    ...
    if (!wifi_reconnect_enabled || wifi_scan_in_progress || wifi_sta_got_ip || wifi_sta_connecting || saved_wifi_ssid[0] == '\0') {
        return;
    }

    ESP_LOGI(TAG, "Wi-Fi reconnect timer: trying saved STA SSID %s", saved_wifi_ssid);
    esp_err_t ret = wifi_sta_apply_and_connect(saved_wifi_ssid, saved_wifi_password);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Wi-Fi saved STA connect start failed: %s", esp_err_to_name(ret));
        wifi_schedule_reconnect(WIFI_RECONNECT_DELAY_MS);
    }
}

static esp_err_t wifi_scan_with_sta_paused(httpd_req_t *req)
{
    wifi_scan_generation++;
    uint32_t this_scan = wifi_scan_generation;
    ...
    /* AP+STA scan policy:
       - If STA is CONNECTING, abort it first because ESP-IDF rejects scans while
         STA is in auth/assoc/connect state.
       - If STA is already CONNECTED and has an IP, do not disconnect it. A scan
         request made through the STA IP must not kill its own HTTP response.
       - If STA is disconnected/half-connected, issue a harmless disconnect to
         clear the driver state before scanning. */
    bool sta_was_connected = wifi_sta_got_ip;

    if (wifi_sta_connecting || (!wifi_sta_got_ip && !wifi_sta_connected)) {
        wifi_ignore_disconnect_event = true;
        ESP_ERROR_CHECK_WITHOUT_ABORT(esp_wifi_disconnect());
        wifi_sta_connecting = false;
    ...
    esp_err_t scan_ret = ESP_FAIL;
    for (int attempt = 0; attempt < 5; attempt++) {
        scan_ret = esp_wifi_scan_start(&scan_config, true);
        if (scan_ret == ESP_OK) break;

        ESP_LOGW(TAG, "Wi-Fi scan_start attempt %d failed: %s", attempt + 1, esp_err_to_name(scan_ret));

        if (scan_ret == ESP_ERR_WIFI_STATE && wifi_sta_connecting) {
            wifi_ignore_disconnect_event = true;
            ESP_ERROR_CHECK_WITHOUT_ABORT(esp_wifi_disconnect());
            wifi_sta_connecting = false;
            vTaskDelay(pdMS_TO_TICKS(500));
        ...
        if (scan_ret != ESP_ERR_WIFI_STATE && scan_ret != ESP_ERR_WIFI_TIMEOUT) break;
    ...
    if (scan_ret != ESP_OK) {
        ...
        if (!wifi_sta_got_ip) {
            wifi_schedule_reconnect(WIFI_SCAN_RECONNECT_PAUSE_MS);
        }
        char err_json[128];
    ...
    snprintf(chunk, sizeof(chunk), "{\"ok\":true,\"scan_id\":%lu,\"sta_connected_during_scan\":%s,\"aps\":[",
             (unsigned long)this_scan,
             sta_was_connected ? "true" : "false");
    esp_err_t ret = httpd_resp_send_chunk(req, chunk, strlen(chunk));
    if (ret != ESP_OK) goto scan_done;
...
static esp_err_t api_wifi_scan_get_handler(httpd_req_t *req)
{
    return wifi_scan_with_sta_paused(req);
}
...
static void wifi_connect_task(void *arg)
{
    ...
    char ssid[33] = {0};
    char password[65] = {0};
    strncpy(ssid, request->ssid, sizeof(ssid) - 1);
    strncpy(password, request->password, sizeof(password) - 1);
    free(request);

    /* Let the HTTP response to /api/wifi/connect leave the socket before the
       STA radio is reconfigured. This matters when the request arrived over
       the current STA interface; disconnecting immediately can kill the
       response and leave the browser/UI in an indeterminate state. */
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Wi-Fi deferred STA connect start failed for %s: %s", ssid, esp_err_to_name(ret));
        wifi_schedule_reconnect(WIFI_RECONNECT_DELAY_MS);
    }
...
static esp_err_t api_wifi_connect_get_handler(httpd_req_t *req)
{
    ...
    /* Do not tear down the current STA connection when the selected SSID is
       already active. The old path disconnected first, which killed the HTTP
       request and made the UI look like scan/connect had failed. */
    if (wifi_sta_got_ip && strncmp(wifi_sta_ssid, ssid, sizeof(wifi_sta_ssid)) == 0) {
        wifi_reconnect_enabled = true;
        wifi_scan_in_progress = false;
        wifi_cancel_reconnect();
        char json[224];
        snprintf(json, sizeof(json),
                 "{\"ok\":true,\"status\":\"already_connected\",\"ssid\":\"%s\",\"ip\":\"%s\",\"saved\":%s}",
                 ssid,
                 wifi_sta_ip,
    ...
    wifi_connect_request_t *request = calloc(1, sizeof(wifi_connect_request_t));
    if (request == NULL) {
    ...
    BaseType_t task_ret = xTaskCreate(wifi_connect_task, "wifi_connect_task", 4096, request, 5, NULL);
    if (task_ret != pdPASS) {
        free(request);
        return httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "failed to start wifi connect task");
    }

    char json[224];
    snprintf(json, sizeof(json), "{\"ok\":true,\"status\":\"connecting\",\"ssid\":\"%s\",\"saved\":%s}",
             ssid,
             save_ret == ESP_OK ? "true" : "false");
    return httpd_resp_sendstr(req, json);
}
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        if (wifi_ignore_disconnect_event) {
            wifi_ignore_disconnect_event = false;
            ESP_LOGI(TAG, "Wi-Fi STA disconnect was intentional; reconnect not scheduled");
        } else if (!wifi_scan_in_progress && wifi_reconnect_enabled) {
            wifi_schedule_reconnect(WIFI_RECONNECT_DELAY_MS);
        }
...
{
    ESP_LOGI(TAG, "Wi-Fi manager build marker: LC_WIFI_SM_2026_05_05_C_ASYNC_CONNECT");
